...
namespace ADS
{
	...
	public class Form1 : System.Windows.Forms.Form
	{
		...
		public enum STATUS {NORMAL,DOWNLOAD,MONITOR,ERRORLOG,SVCTOOL,ESMI_DOWNLOAD,GRAPH,RING};
		...
		private void InitializeComponent()
		{
			...
			// Form1
			// 
			this.ClientSize = new System.Drawing.Size(234, 755);
...
//
		...
		public bool ReadBuff()
		{
			...
			{
				if(!state.Equals(STATUS.MONITOR) && !state.Equals(STATUS.GRAPH) && !state.Equals(STATUS.RING)) // 모니터링 모드나 그래프 모드나 RING 모드가 아니면
				{
				...
				if(state==Form1.STATUS.RING)
				{
					...
					for(int i=0;i<dwBytes;i++)
					{
						...
						if((ring_rx_buf[ring_rx_ptw] & 0x80)==0) // if last byte
						{
							...
							SendBytes(txData);
							if((txData[0]&0x70)==0x10 && (txData[1]&0x60)==0x20)
							{
								int floor=txData[4]&0x7f;
								ringMon.setFloor(floor);
							}
						}
						ring_rx_ptw++;
					}
