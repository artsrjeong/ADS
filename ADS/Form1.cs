...
//
		...
		;********************************************************************/
		public byte  M_get_type (byte[] msg_ptr)
		{
			return((byte)((msg_ptr[0] & 0x70) >> 4));
		}
		...
		;***************************************************************************/
		public byte  M_get_subtype (byte[] msg_ptr)
		{
			return((byte)((msg_ptr[1] & 0x60) >> 5));
		}
		...
		{
			if(this.state.Equals(STATUS.RING) && ring.tabControl1.SelectedIndex==2 && ring.btnCapture.Text.Equals("Stop"))
			{
				this.timer1.Enabled=false;
				while(ring.btnCapture.Text.Equals("Stop"))
				{
					ReadBuff();
		...
		public void AddCaptureSec()
		{
			...
			if(captureSec>=3600)
			{
				hour=captureSec/3600;
			}
			rest=captureSec%3600;
			min=rest/60;
			sec=rest%60;
			ring.lbTime.Text="Time : "+hour.ToString()+":"+min.ToString()+":"+sec.ToString();
		}
		...
		public bool ReadBuff()
		{
			...
			else
			{
				...
				if(state==Form1.STATUS.RING)
				{
					...
					int type,subType,filterType,filterSubType;
					for(int i=0;i<dwBytes;i++)
					{
						...
						if((ring_rx_buf[ring_rx_ptw] & 0x80)==0) // if last byte
						{
							...
							{
								//if filtering
								if(ring.tabControl1.SelectedIndex==1 && ring.btnFilter.Text.Equals("Stop"))
								{
									type=this.M_get_type(txData);
									if(type<5)
										subType=this.M_get_subtype(txData);
									else
										subType=255;
									for(int rowInx=0;rowInx<2;rowInx++)
									{
										string strType,strSubType;
										strType=(string)ring.dtFilter.Rows[rowInx][0];
										
										if(strType.Length>0)
										{
											filterType=Int32.Parse(strType);
											filterSubType=255;
											strSubType=(string)ring.dtFilter.Rows[rowInx][1];
											if(strSubType.Length>0 && filterType<5)
											{
												filterSubType=Int32.Parse(strSubType);
											}
											if(filterType==type && (filterSubType==255 || filterSubType==subType))
											{
												string strPkt=string.Format("{0:X2},{1:X2},{2:X2},{3:X2},{4:X2},{5:X2}",txData[0],txData[1],txData[2],txData[3],txData[4],txData[5]);
												ring.dtFilterResult.Rows.Add(new object[]{type.ToString(),subType.ToString(),strPkt});
											}
										}
									}
								}
								...
								{
									if(ring.tabControl1.SelectedIndex==0 && !ring.pnRcv1.BackColor.Equals(Color.Gray))
									{
										ring.pnRcv1.BackColor=Color.Gray;
									}
									if(ring.tabControl1.SelectedIndex==1 && !ring.pnRcv2.BackColor.Equals(Color.Gray))
									{
										ring.pnRcv2.BackColor=Color.Gray;
									}
									if(ring.tabControl1.SelectedIndex==2 && !ring.pnRcv3.BackColor.Equals(Color.Gray) && ring.btnCapture.Text.Equals("Stop"))
									{
										ring.pnRcv3.BackColor=Color.Gray;	
										AddCaptureSec();
									}
								}
								else
								{
									if(ring.tabControl1.SelectedIndex==0 && !ring.pnRcv1.BackColor.Equals(Color.Red))
									{
										ring.pnRcv1.BackColor=Color.Red;
									}
									if(ring.tabControl1.SelectedIndex==1 && !ring.pnRcv2.BackColor.Equals(Color.Red))
									{
										ring.pnRcv2.BackColor=Color.Red;
									}
									if(ring.tabControl1.SelectedIndex==2 && !ring.pnRcv3.BackColor.Equals(Color.Red) && ring.btnCapture.Text.Equals("Stop"))
									{
										ring.pnRcv3.BackColor=Color.Red;	
										AddCaptureSec();
									}
								}
								SendBytes(txData);
								if((txData[0]&0x70)==0x00)
								{
									...
									if((txData[1]&0x60)==0x20 && ring.tabControl1.SelectedIndex==0) //monitor
									{
										int floor=txData[4]&0x7f;
										ring.setFloor(car,floor);
										int dir=(txData[3]&0x30)>>4;
										ring.setDir(car,dir);
										int fDoor=txData[3]&0x03;
										ring.setDoor(car,fDoor);
									}
								}

								try
								{
									if(ring.tabControl1.SelectedIndex==2 && ring.btnCapture.Text.Equals("Stop"))
									{
										int origin=txData[0]&0x0f;
										type=(txData[0]&0x70)>>4;
